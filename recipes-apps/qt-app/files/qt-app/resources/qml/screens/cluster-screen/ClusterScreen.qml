	...
	Item {
		...
		ColumnLayout {
			...
			Item {
				...
				// =========================================================
				// ROAD CONTAINER: Deslocamento Lateral Geométrico e Curvatura 3D
				// =========================================================
				Item {
					id: roadContainer
					...
					// DESLOCAMENTO LATERAL (Aplica-se à base da grelha e das linhas)
					...
					// CURVATURA (Desloca apenas o topo das linhas da estrada para simular perspetiva)
					property real currentCurveOffset: (root.vehicleDataAvailable && vehicleData.laneHeading !== undefined) ? (vehicleData.laneHeading * -600 * root.sx) : 0

					// Interpoladores para garantir os 60 FPS fluídos
					Behavior on currentLateralOffset {
					...
					Image {
						...
						anchors.fill: parent
						// A grelha move-se fisicamente no eixo X para acompanhar o carro
						anchors.leftMargin: roadContainer.currentLateralOffset
						anchors.rightMargin: -roadContainer.currentLateralOffset
						opacity: AppTheme.isDark ? 0.35 : 0.10

						// O tapete base torce ligeiramente com a curva para dar ilusão 3D
						transform: Rotation {
					...
					Shape {
						...
						ShapePath {
							...
							// Começa no canto inferior esquerdo (afetado pelo offset lateral)
							startX: (roadContainer.width / 2) - (350 * root.sx) + roadContainer.currentLateralOffset
							startY: roadContainer.height

							// Curva para o canto superior esquerdo (afetado apenas pela curva, simulando o horizonte fixo)
							PathQuad {
								x: (roadContainer.width / 2) - (80 * root.sx) + roadContainer.currentCurveOffset
								y: roadContainer.height * 0.25
								controlX: (roadContainer.width / 2) - (150 * root.sx) + (roadContainer.currentCurveOffset * 0.6) + (roadContainer.currentLateralOffset * 0.5)
							...
							// Curva para o canto inferior direito (voltamos à base, afetada pelo offset lateral)
							PathQuad {
								x: (roadContainer.width / 2) + (350 * root.sx) + roadContainer.currentLateralOffset
						...
						ShapePath {
							...
							// Base esquerda (afetada pelo offset lateral)
							startX: (roadContainer.width / 2) - (350 * root.sx) + roadContainer.currentLateralOffset
							startY: roadContainer.height
						...
						ShapePath {
							...
							// Base direita (afetada pelo offset lateral)
							startX: (roadContainer.width / 2) + (350 * root.sx) + roadContainer.currentLateralOffset
							startY: roadContainer.height
